...
class dynamicResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000):
        ...
        self.relu = nn.ReLU(inplace=True)
        self.maxpool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)
        self.layer1 = self._make_layer(BasicBlock, 64, 56, layers[0])
    ...
    def _make_layer(self, block, planes,spatial, blocks, stride=1):
        downsample = None
        if stride != 1 or self.inplanes != planes * block.expansion:
            downsample = nn.Sequential(
                nn.Conv2d(self.inplanes, planes * block.expansion,
        ...
        layers = []
        layers.append(block(self.inplanes, planes,spatial, stride, downsample))
        self.inplanes = planes * block.expansion
        for i in range(1, blocks):
            layers.append(block(self.inplanes, planes,spatial))
